<?php
  ...
  $sql="SELECT f.Customer, year(date), sum(revenue) from f_gbi f join d_customer d on f.Customer = d.Customer where f.CUstomer = 5000 group by d.Customer, year(date)";
...
<html>
  <head>
    <meta charset="utf-8">
    <title>Philly Bikes Verkauf</title>
  ...
  <a-assets>
	...
	<a-entity position=" -1 0 -7" scale ="0.5 0.5 0.5" rotation="0 0 0 ">
	<?php
	  ...
	  while ($row = $res->fetch_row()) {
		...
		$year=$row[1];
		$z=$year-2016;
		print "<a-box position=\"$z $v2 $x\" rotation=\"0 0 0\" scale=\"0.6 $val 0.6\" color=#F0F0EC shadow></a-box>\n";
		// print "<a-box position=\"$x $v2 $z\" rotation=\"0 0 0\" scale=\"0.6 $val 0.6\" color=#F0F0EC change-color-on-hover="color: pink" shadow></a-box>\n";
	  }
	?>
	</a-entity>

	<!-- Diagrammtext -->
		<a-entity position="0 -0.7 -6.3" material="color: #eee" text="color: white; align: center; value: Jahr; width: 6">
        </a-entity>
		...
		<a-entity position="0 9 -10" geometry="primitive: plane; width: 4; height: 1" material="color: #fff" text="color: blue; align: center; value: Beantown Bikes; width: 10">
        </a-entity>
